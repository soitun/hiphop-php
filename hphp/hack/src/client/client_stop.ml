...
let nice_kill env =
  ...
  try
    match Monitor_connection.connect_and_shut_down ~tracker env.root with
    | Ok shutdown_result -> begin
      match shutdown_result with
      | Monitor_utils.SHUTDOWN_VERIFIED ->
        Printf.eprintf "Successfully killed server for %s\n%!" root_s
      | Monitor_utils.SHUTDOWN_UNVERIFIED ->
        Printf.eprintf
