...
type diagnostics_from =
  | Diagnostics_from_clientIdeDaemon of {
      diagnostics: Client_ide_message.diagnostic list;
      validated: bool;
    }
...
(** The architecture of clientLsp is a single-threaded message loop inside
  [Client_lsp.main]. The loop calls [get_next_event] to wait for the next
...
type event =
  ...
  | Daemon_notification of Client_ide_message.notification
...
let event_to_string (event : event) : string =
  ...
  | Daemon_notification n ->
    Printf.sprintf
      ...
      (Client_ide_message.notification_to_string n)
  | Tick -> "Tick"
...
let pop_from_ide_service (ide_service : Client_ide_service.t) : event Lwt.t =
  let%lwt notification_opt =
    Lwt_message_queue.pop (Client_ide_service.get_notifications ide_service)
  in
...
let get_client_message_source
    ...
    (ide_service : Client_ide_service.t)
    (errors_q_opt :
      ( Server_progress.ErrorsRead.read_result,
        Server_progress_lwt.watch_error )
      result
...
let get_next_event
    (state : state ref)
    (client : Jsonrpc.t)
    (ide_service : Client_ide_service.t) : event Lwt.t =
  let can_use_client =
    ...
    | Some { Initialize.initializationOptions; _ }
      ...
      match Client_ide_service.get_status ide_service with
      | Client_ide_service.Status.(Initializing | Rpc _) -> false
      | Client_ide_service.Status.(Ready | Stopped _) -> true
    end
    | _ -> true
  in
...
let ide_diagnostics_to_lsp_diagnostics
    (filename : string) (diagnostics : Client_ide_message.diagnostic list) :
  ...
  let ide_diagnostic_to_lsp_diagnostics
      (diagnostic : Client_ide_message.diagnostic) :
      Lsp.PublishDiagnostics.diagnostic list =
    let Client_ide_message.{ diagnostic_error; diagnostic_hash; _ } =
      diagnostic
    in
    let {
    ...
    let additional_diagnostics =
      List.map
        diagnostic.Client_ide_message.diagnostic_related_hints
        ~f:(fun hint_pos ->
          let { uri = _; range } =
...
let lsp_document_to_ide (text_document : TextDocumentItem.t) :
  ...
  {
    Client_ide_message.file_path = Path.make @@ Lsp_helpers.lsp_uri_to_path fn;
...
let get_documents_from_open_files
    ...
    Client_ide_message.document list =
...
let get_document_location
    ...
    Client_ide_message.document * File_content.Position.t =
  ...
  ({ Client_ide_message.file_path; file_contents }, pos)
...
let start_server () : unit =
  ...
  in
  let _exit_status = Client_start.main env_start in
  ()

let announce_ide_failure (error_data : Client_ide_message.rich_error) :
    unit Lwt.t =
...
let run_ide_service
  ...
  let naming_table_load_info =
    ...
    | (_, Some path) ->
      Some
        {
          Client_ide_message.Initialize_from_saved_state.path = Path.make path;
          test_delay =
  ...
  let%lwt result =
    Client_ide_service.initialize_from_saved_state
      ide_service
      ~root:!env.root
  ...
  match result with
  | Ok () ->
    let%lwt () = Client_ide_service.serve ide_service in
    Lwt.return_unit
  | Error error_data ->
...
let stop_ide_service
    (ide_service : Client_ide_service.t)
    ~(tracking_id : string)
    ~(stop_reason : Client_ide_service.Stop_reason.t) : unit Lwt.t =
  log
    "Stopping IDE service process: %s"
    (Client_ide_service.Stop_reason.to_log_string stop_reason);
  let%lwt () =
    Client_ide_service.stop ide_service ~tracking_id ~stop_reason ~e:None
  in
  Lwt.return_unit
...
let merge_statuses_standalone
    ...
    (errors : errors_conn)
    (ide : Client_ide_service.t) : Lsp.ShowStatusFB.params =
  let ( (client_ide_disposition, client_ide_is_initializing),
        client_ide_message,
        client_ide_tooltip ) =
    match Client_ide_service.get_status ide with
    | Client_ide_service.Status.Initializing ->
      ( (Server_progress.DWorking, true),
        "Hack: initializing",
        "Hack IDE support is initializing (loading saved state)" )
    | Client_ide_service.Status.Rpc _ ->
      ( (Server_progress.DWorking, false),
        "Hack",
        "Hack is working on IDE requests" )
    | Client_ide_service.Status.Ready ->
      ((Server_progress.DReady, false), "Hack", "Hack IDE support is ready")
    | Client_ide_service.Status.Stopped s ->
      ( (Server_progress.DStopped, false),
        "Hack: " ^ s.Client_ide_message.short_user_message,
        s.Client_ide_message.medium_user_message ^ see_output_hack )
  in
...
let refresh_status ~(ide_service : Client_ide_service.t) : unit =
  match !initialize_params_ref with
...
let background_status_refresher (ide_service : Client_ide_service.t) : unit =
  let rec loop () =
...
let ide_rpc
    ...
    (message : 'a Client_ide_message.t) : 'a Lwt.t =
  let%lwt result =
    Client_ide_service.rpc
      ide_service
      ~tracking_id
      ~ref_unblocked_time
...
let do_shutdown
    (state : state) (ide_service : Client_ide_service.t) (tracking_id : string)
    : state Lwt.t =
  ...
  let%lwt () =
    stop_ide_service
      ide_service
      ~tracking_id
      ~stop_reason:Client_ide_service.Stop_reason.Editor_exited
  in
...
let do_hover
  ...
  let%lwt infos =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      (Client_ide_message.Hover (document, location))
  in
...
let do_typeDefinition
  ...
  let%lwt results =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      (Client_ide_message.Type_definition (document, location))
  in
  let file = Path.to_string document.Client_ide_message.file_path in
  let results =
...
let do_definition
  ...
  let%lwt results =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      (Client_ide_message.Definition (document, location))
  in
  let locations =
    List.map results ~f:(fun (_, definition) ->
        hack_symbol_definition_to_lsp_identifier_location
          definition
          ~default_path:(document.Client_ide_message.file_path |> Path.to_string))
  in
...
let do_completion
  ...
  let request =
    Client_ide_message.Completion
      (document, location, { Client_ide_message.is_manually_invoked })
  in
  let%lwt infos =
    ide_rpc ide_service ~tracking_id ~ref_unblocked_time request
  in
  let filename = document.Client_ide_message.file_path |> Path.to_string in
    ...
    let%lwt result =
      try
        ...
        | Some _ ->
          ...
          let request =
            Client_ide_message.Completion_resolve_location
              ( file_path,
                Client_ide_message.Full_name fullname,
                pos,
          ...
          let documentation =
            docblock_with_ranking_detail
              raw_docblock.Client_ide_message.Completion_resolve.docblock
              ranking_detail
            |> docblock_to_markdown
          in
          (match
             raw_docblock.Client_ide_message.Completion_resolve.signature
           with
      ...
      | _ ->
        ...
        let request =
          Client_ide_message.Completion_resolve
            Client_ide_message.Completion_resolve.{ fullname; kind }
        in
        ...
        let documentation =
          docblock_to_markdown
            raw_docblock.Client_ide_message.Completion_resolve.docblock
        in
        (match raw_docblock.Client_ide_message.Completion_resolve.signature with
        | None -> Lwt.return { params with Completion.documentation }
        | Some signature ->
          Lwt.return
            { params with Completion.documentation; detail = Some signature })
...
let do_workspaceSymbol
  ...
  let request = Client_ide_message.Workspace_symbol query in
  let%lwt results =
    ide_rpc ide_service ~tracking_id ~ref_unblocked_time request
  in
...
let do_documentSymbol
  ...
  let request = Client_ide_message.Document_symbol document in
  let%lwt outline =
    ide_rpc ide_service ~tracking_id ~ref_unblocked_time request
  in
  let converted =
...
let do_findReferences
  ...
  let%lwt response =
    ide_rpc
      ...
      ~ref_unblocked_time
      (Client_ide_message.Find_references
         (document, location, all_open_documents))
  in
  match response with
  | Client_ide_message.Invalid_symbol ->
...
let do_goToImplementation
  ...
  let%lwt response =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      (Client_ide_message.Go_to_implementation
         (document, location, all_open_documents))
  in
  match response with
  | Client_ide_message.Invalid_symbol_impl ->
...
let do_highlight
  ...
  let%lwt ranges =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      (Client_ide_message.Document_highlight (document, location))
  in
...
let location_of_code_action_request
    ...
    Client_ide_message.document * Ide_api_types.range * Path.t =
  let text_document =
  ...
  in
  let document = lsp_document_to_ide text_document in
  let file_path = document.Client_ide_message.file_path in
...
let do_codeAction
  ...
  let%lwt (actions, errors_opt) =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      (Client_ide_message.Code_action (document, range))
  in
...
let do_codeAction_resolve
  ...
  let%lwt result =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      (Client_ide_message.Code_action_resolve
...
let do_signatureHelp
  ...
  let%lwt signatures =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      (Client_ide_message.Signature_help (document, location))
  in
...
let do_topLevelDefNameAtPos
  ...
  let%lwt result =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      (Client_ide_message.Top_level_def_name_at_pos (document, location))
  in
  Lwt.return result
...
let do_documentRename
  ...
  let%lwt response =
    ide_rpc
      ...
      ~tracking_id:metadata.tracking_id
      ~ref_unblocked_time
      (Client_ide_message.Rename
         (document, location, new_name, all_open_documents))
  in
  match response with
  | Client_ide_message.Not_renameable_position ->
    let message = "Tried to rename a non-renameable symbol" in
    raise
      (Error.LspException
         { Error.code = Error.InvalidRequest; message; data = None })
  | Client_ide_message.Rename_success { shellout = None; local } ->
...
let do_autoclose
  ...
  let%lwt result =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      (Client_ide_message.AutoClose (document, location))
  in
  Lwt.return result
...
let report_recheck_telemetry
    ...
    ~(ref_unblocked_time : float ref)
    (uri : DocumentUri.t)
    (result : (Client_ide_message.diagnostic list, unit) Result.t) : unit =
...
(** This will run [f] but wrap it up in a way that respects cancellation:
  ...
  the invariant -- we have only a single caller, the main Client_lsp message-loop,
...
let send_file_to_ide_and_defer_check
  ...
  let%lwt _errors =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      Client_ide_message.(Did_open_or_change { file_path; file_contents })
  in
  match !state with
  | Running renv ->
  ...
  | NotificationMessage (DidCloseNotification params) ->
    ...
    let%lwt diagnostics =
      ide_rpc
        ...
        ~tracking_id:metadata.tracking_id
        ~ref_unblocked_time
        (Client_ide_message.Did_close file_path)
    in
    state :=
...
let set_verbose_to_file
    ~(ide_service : Client_ide_service.t) ~(tracking_id : string) (value : bool)
    : unit Lwt.t =
  ...
  let%lwt () =
    ide_rpc
      ...
      ~tracking_id
      ~ref_unblocked_time
      (Client_ide_message.Verbose_to_file !verbose_to_file)
  in
  Lwt.return_unit
...
let handle_client_message
  ...
  let%lwt result_telemetry_opt =
    ...
    | (_, RequestMessage (id, HackTestShutdownServerlessRequestFB)) ->
      let%lwt () =
        stop_ide_service
          ide_service
          ~tracking_id
          ~stop_reason:Client_ide_service.Stop_reason.Testing
      in
      respond_jsonrpc
        ~powered_by:Serverless_ide
        id
    ...
    | (_, RequestMessage (id, HackTestStopServerRequestFB)) ->
      Client_stop.kill_server !env.root !env.args.from;
    ...
    | (_, NotificationMessage (DidChangeWatchedFilesNotification notification))
      ...
      let%lwt () =
        ide_rpc
          ...
          ~tracking_id
          ~ref_unblocked_time
          (Client_ide_message.Did_change_watched_files changes)
      in
      Lwt.return_some
...
let handle_daemon_notification
    ~(state : state ref)
    ~(ide_service : Client_ide_service.t)
    ~(notification : Client_ide_message.notification)
    ~(ref_unblocked_time : float ref) : result_telemetry option Lwt.t =
  ...
  | Client_ide_message.Done_init (Ok ()) ->
    ...
    in
    Lwt.return_none
  | Client_ide_message.Done_init (Error error_data) ->
    log_debug "<-- done_init";
  ...
  | Running renv ->
    ...
    let%lwt diagnostics =
      ide_rpc
        ...
        ~tracking_id
        ~ref_unblocked_time
        Client_ide_message.(Diagnostics { file_path; file_contents })
    in
    let new_state =
      publish_and_report_after_recomputing_live_squiggles
        !state
