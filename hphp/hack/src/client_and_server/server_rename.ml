...
let get_lambda_parameter_rewrite_patches ctx files =
  List.concat_map files ~f:(fun file ->
      Server_rewrite_lambda_parameters.get_patches
        ctx
...
let go
  ...
  Server_find_refs.go
    ...
    genv
    env
  |> Server_command_types.Done_or_retry.map_env ~f:(fun refs ->
         let changes =
...
let go_for_single_file
  ...
  let action =
    ...
    | Server_command_types.Find_refs.Class str ->
      (* Note that [ServerRename.go] remaps ClassRename to Find_refs.ExplicitClass,
         so we manually handle that here for parity.
      *)
      Server_command_types.Find_refs.ExplicitClass str
    | action -> action
  in
  ( Server_find_refs.go_for_single_file ~ctx ~action ~filename |> fun refs ->
    let changes =
      List.fold_left
        refs
        ~f:
