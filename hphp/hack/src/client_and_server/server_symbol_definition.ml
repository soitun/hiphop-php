...
let get_member_def (ctx : Provider_context.t) (x : class_element) =
  ...
  | Static_method ->
    ...
    >>= fun m -> Some (File_outline.summarize_method member_origin m)
  | Property
  | Static_property ->
    let props =
    ...
    >>= fun p -> Some (File_outline.summarize_property member_origin p)
  | Class_const ->
    ...
    in
    Option.map ~f:(File_outline.summarize_class_const member_origin) res
  | Typeconst ->
    let tconsts = c.c_typeconsts in
    List.find tconsts ~f:(fun t ->
        String.equal (snd t.c_tconst_name) member_name)
    >>= fun t -> Some (File_outline.summarize_class_typeconst member_origin t)
...
let get_local_var_def ast name p =
  ...
  let def = List.hd (Server_find_locals.go_from_ast ~ast pos) in
  Option.map def ~f:(File_outline.summarize_local name)
...
let summarize_class_typedef ctx x =
  ...
  | Naming_types.TClass ->
    ...
    Some (File_outline.summarize_class c ~no_children:true)
  | Naming_types.TTypedef ->
    Ast_provider.find_typedef_in_file ctx fn x ~full:false >>= fun tdef ->
    Some (File_outline.summarize_typedef tdef)
...
let go ctx ast (result : _ Symbol_occurrence.t) : _ Symbol_definition.t option =
  ...
  | SO.Function ->
    ...
    Some (File_outline.summarize_fun f)
  | SO.GConst ->
    get_gconst_by_name ctx result.SO.name >>= fun cst ->
    Some (File_outline.summarize_gconst cst)
  ...
  | SO.TypeVar ->
    (match ast with
    | None -> None
    | Some ast -> Server_find_type_var.go ast result.SO.pos result.SO.name)
  | SO.HhFixme -> None
  | SO.HhIgnore -> None
  | SO.Module ->
    get_module_def_by_name ctx result.SO.name >>= fun md ->
    Some (File_outline.summarize_module_def md)
