...
let get_autocomplete_context
  ...
  if File_content.Position.is_beginning_of_line pos then
    {
      Autocomplete_types.is_manually_invoked;
    ...
    let char_at_pos =
      try
        file_content.[offset + Autocomplete_types.autocomplete_token_length]
      with
      | _ -> ' '
    in
...
let go_ctx
    ...
    ~(naming_table : Naming_table.t)
    ~(is_manually_invoked : bool)
    pos : Autocomplete_types.ide_result =
  (* Be sure to fix tcopt on all entry points of this file *)
  let ctx = Provider_context.with_tcopt_for_autocomplete ctx in
      ...
      File_content.
        {
          ...
          text = Autocomplete_types.autocomplete_token;
        };
    ]
  in
  ...
  in
  {
    Autocomplete_types.completions = result.Utils.With_complete_flag.value;
    char_at_pos = autocomplete_context.Autocomplete_types.char_at_pos;
