  ...
  let rec traverse nullable dynamic tyl_acc tyl =
    ...
    | ty :: tyl ->
      if Utils.is_nothing env ty then
        ...
        | (r, Tdynamic None) -> traverse nullable (Some r) tyl_acc tyl
        | (_, Tdynamic (Some _)) ->
          traverse nullable dynamic (ty :: tyl_acc) tyl
          ...
          let (env, inter_ty) =
            try
              ...
              | ((_, Tclass_ptr ty_c), (_, Tnewtype (cn, [ty_cn], _)))
                when String.equal
                       cn
                       Naming_special_names.Classes.cClassOrClassname ->
                let (env, ty) = intersect ~r env rec_tracker ty_c ty_cn in
                (env, MkType.class_or_classname r [ty])
              | ( (_, Tnewtype (cn1, [ty_cn1], _)),
                ...
                when String.equal
                     ...
                     || String.equal cn1 Naming_special_names.Classes.cClassname
                        && String.equal
                             cn2
                             Naming_special_names.Classes.cClassOrClassname ->
