...
let get_sockaddr config =
  let sock_name =
    Socket.make_valid_socket_path config.Monitor_utils.socket_file
  in
...
let connect_to_monitor
  ...
  let phase = ref Monitor_utils.Connect_open_socket in
    ...
    ~f:(fun () ->
      try
        ...
        Timeout.with_timeout
          ...
          ~do_:(fun _timeout ->
            (* 1. open the socket *)
            phase := Monitor_utils.Connect_open_socket;
            ...
            phase := Monitor_utils.Connect_send_version;
            let version_str =
              Monitor_utils.VersionPayload.serialize
                ~tracker
                ~terminate_monitor_on_version_mismatch
            in
            let (_ : int) =
              Marshal_tools.to_fd_with_preamble
                ...
                version_str
            in
            phase := Monitor_utils.Connect_send_newline;
            let (_ : int) =
              Unix.write
                (Unix.descr_of_out_channel oc)
                (Bytes.of_string "\n")
            ...
            phase := Monitor_utils.Connect_receive_connection_ok;
            ...
            | Build_id_mismatch_v3 (mismatch_info, mismatch_payload) ->
              let monitor_will_terminate =
                match
                  Monitor_utils.MismatchPayload.deserialize mismatch_payload
                with
                | Error msg ->
                  Hh_logger.log
                    ...
                    mismatch_payload;
                  true
                | Ok { Monitor_utils.MismatchPayload.monitor_will_terminate } ->
  ...
  Timeout.with_timeout
    ...
    ~on_timeout:(fun _timings ->
      if server_exists config.lock_file then
        Ok Monitor_utils.SHUTDOWN_UNVERIFIED
      else
        Error
          (Connect_to_monitor_failure
             {
    ...
    ~do_:
      begin
        fun _ ->
          wait_on_server_restart ic;
          Ok Monitor_utils.SHUTDOWN_VERIFIED
      end
...
let connect_once
  ...
  let result =
    ...
    Timeout.with_timeout
      ...
      ~on_timeout:(fun _ -> Error Monitor_utils.Server_dormant_out_of_retries)
  in
  (* oops too heavy *)
